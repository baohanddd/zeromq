...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  config.vm.provision "shell", inline: <<-shell
    ...
    apt-get update
    apt-get install libtool autoconf automake uuid-dev e2fsprogs build-essential -y
    tar -zxvf /vagrant/zeromq-4.0.5.tar.gz
    cd ~/zeromq-4.0.5 && ./configure && make && make install
    rm -rf ~/zeromq-4.0.5
    apt-get install php5-cli php-pear php5-dev libcurl3-openssl-dev -y
